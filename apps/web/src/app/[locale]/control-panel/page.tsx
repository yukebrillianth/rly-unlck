...
import { GaugeIcon, InfinityIcon, KeySquareIcon, SettingsIcon, UsersIcon } from "lucide-react";
...
export default async function AdminPage() {
	const { userCount } = await loadData();
							...
							<Link href="/control-panel/users">
								<div className="flex justify-between">
									...
									<div className="text-muted-foreground text-sm">
										<span
											className={cn({
												"text-destructive": userLimit !== null && userCount > userLimit,
											})}
										>
											<Trans
												i18nKey="userCount"
						...
						<Tile asChild>
							<Link href="/control-panel/license">
								<div className="flex justify-between">
									...
									<span className="text-primary text-sm capitalize">{tier}</span>
								</div>
								<TileTitle>
