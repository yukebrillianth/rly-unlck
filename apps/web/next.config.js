...
const nextConfig = {
  ...
  // Use assetPrefix only when explicitly set to a non-localhost host. Using
  // http://localhost for clients that are not on the same machine breaks asset loading
  // (ERR_CONNECTION_REFUSED) because the browser resolves to the viewer's localhost.
  assetPrefix:
    process.env.NEXT_PUBLIC_SELF_HOSTED === "true" &&
    process.env.NEXT_PUBLIC_BASE_URL &&
    !/^(https?:\/\/(localhost|127\.0\.0\.1))(\b|:)/.test(
      process.env.NEXT_PUBLIC_BASE_URL,
    )
      ? normalizeToHttps(process.env.NEXT_PUBLIC_BASE_URL)
      : undefined,
  webpack(config) {
